...
def test_update_check_results_for_page():
    ...
    wcag_check_result_initials: QuerySet[WcagCheckResultInitial] = (
        WcagCheckResultInitial.objects.filter(wcag_page_initial=wcag_page_initial)
    )

    number_of_forms: int = len(wcag_check_result_initials) + 1
    ...
    for count, wcag_check_result_initial in enumerate(wcag_check_result_initials):
    ...
    new_wcag_definition: WcagDefinition = WcagDefinition.objects.get(
    ...
    assert (
        wcag_audit.simplified_case.audit_overview.published_report_data_updated_time
        is None
    )
    ...
    updated_check_result: WcagCheckResultInitial = WcagCheckResultInitial.objects.get(
    ...
    assert (
        updated_wcag_audit.simplified_case.audit_overview.published_report_data_updated_time
        is not None
    )
...
def test_get_all_possible_check_results_for_page():
    ...
    pdf_wcag_definitions: list[WcagDefinition] = list(
        WcagDefinition.objects.filter(type=WcagDefinition.Type.PDF)
    )

    assert len(pdf_wcag_definitions) > 3
...
def test_get_other_pages_with_retest_notes():
    ...
    wcag_page_retest.url = "https://example.com"
    wcag_page_retest.notes = "Primary"
    wcag_page_retest.save()
    other_page_with_retest_notes: WcagPageRetest = WcagPageRetest.objects.get(
        wcag_audit=twelve_week_wcag_audit,
        wcag_page_initial__page_type=WcagPageInitial.Type.CONTACT,
    )
    other_page_with_retest_notes.url = "https://example.com"
...
def test_get_audit_summary_wcag_check_results_by_page(rf):
    ...
    assert (
        ...
        in summary_wcag_check_results_by_page
    )
    assert (
        len(
            summary_wcag_check_results_by_page[
    ...
    assert (
        summary_wcag_check_results_by_page[wcag_check_result_initial.wcag_page_initial][
            0
        ].initial_result
        == wcag_check_result_initial
    )
...
def test_get_audit_summary_pages_with_retest_notes(rf):
    ...
    assert context["pages_with_retest_notes"].first() == wcag_page_retest
...
def test_get_audit_summary_wcag_check_results_by_wcag(rf):
    ...
    assert "summary_wcag_check_results_by_wcag" in context
    assert context["summary_wcag_check_results_by_wcag"] == {}
    ...
    assert (
        wcag_check_result_initial.wcag_definition in summary_wcag_check_results_by_wcag
    )
    assert (
        len(
            summary_wcag_check_results_by_wcag[
                wcag_check_result_initial.wcag_definition
            ]
        )
        == 1
    )
    assert (
        summary_wcag_check_results_by_wcag[wcag_check_result_initial.wcag_definition][
            0
        ].initial_result
        == wcag_check_result_initial
    )
...
def test_get_audit_summary_issue_counts(rf):
    ...
    assert "number_of_statement_issues" in context
    assert context["number_of_statement_issues"] == 44
...
def test_add_wcag_check_result_initial_notes_history():
    ...
    assert wcag_check_result_initial_notes_history.notes == NEW_CHECK_RESULT_NOTES
    assert wcag_check_result_initial_notes_history.created_by == user
...
def test_wcag_check_result_initial_notes_history_changed():
    ...
    assert WcagCheckResultInitialNotesHistory.objects.all().count() == 0
    ...
    assert WcagCheckResultInitialNotesHistory.objects.all().count() == 1
...
def test_add_check_result_restest_notes_history():
    ...
    assert wcag_check_result_retest_notes_history.notes == NEW_RETEST_NOTES
    assert wcag_check_result_retest_notes_history.created_by == user
...
def test_check_result_restest_notes_history_changed():
    ...
    assert WcagCheckResultRetestNotesHistory.objects.all().count() == 0
