...
def test_case_report_correspondence_view_contains_followup_due_dates(admin_client):
    ...
    assertContains(
        response,
        f'<div class="govuk-hint">Due {gds_date(ONE_WEEK_FOLLOWUP_DUE_DATE)}</div>',
    )
    assertContains(
        response,
        f'<div class="govuk-hint">Due {gds_date(FOUR_WEEK_FOLLOWUP_DUE_DATE)}</div>',
    )
    assertContains(
        response,
        f'<div class="govuk-hint">Due {gds_date(TWELVE_WEEK_FOLLOWUP_DUE_DATE)}</div>',
        html=True,
    )
