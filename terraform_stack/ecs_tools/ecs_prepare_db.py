"""ECS Prepare DB - Launches on ECS that prepares the database for the prototype"""
...
def most_recent_db_s3_path(bucket: str) -> str:
    ...
    db_backups: list[dict[str, Any]] = []

    for obj in s3_client.list_objects(Bucket=bucket)["Contents"]:
        if ".sql" in obj["Key"]:
            db_backups.append(obj)

    db_backups.sort(key=lambda obj: obj["LastModified"])

    return str(db_backups[-1]["Key"])
